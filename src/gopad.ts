...
class Gopad {
  ...
  private outstanding?: ot.Sequence;
  private buffer?: ot.Sequence;
  ...
  constructor(readonly options: GopadOptions) {
  ...
  /** Destroy this Gopad instance and close any sockets. */
  dispose() {
    ...
    if (msg.Identity !== undefined) {
      ...
      for (let i = this.revision - start; i < operations.length; i++) {
        ...
        if (id === this.me) {
          ...
          operation = ot.FromString(JSON.stringify(operation));
  ...
  private applyClient(operation: ot.Sequence) {
    if (!this.outstanding) {
  ...
  private applyOperation(operation: ot.Sequence) {
  ...
  private transformCursors(operation: ot.Sequence) {
    for (const data of Object.values(this.userCursors)) {
      data.cursors = data.cursors.map((c) => operation.TransformIndex(c));
      data.selections = data.selections.map(([s, e]) => [
        operation.TransformIndex(s),
        operation.TransformIndex(e),
      ]);
    }
    this.updateCursors();
  }
