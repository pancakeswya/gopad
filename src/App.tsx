...
import Gopad, { UserInfo } from "./gopad";
...
function App() {
  ...
  useEffect(() => {
    if (editor?.getModel()) {
      ...
      return () => {
        gopad.current?.dispose();
        gopad.current = undefined;
      };
    }
  ...
  useEffect(() => {
    if (connection === "connected") {
      gopad.current?.setInfo({ name, hue });
    }
  }, [connection, name, hue]);

  function handleLanguageChange(language: string) {
    setLanguage(language);
    if (gopad.current?.setLanguage(language)) {
      toast({
